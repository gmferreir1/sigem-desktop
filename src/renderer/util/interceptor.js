...
http.interceptors.response.use(
  ...
  function(error) {
    ...
    if (error.response.status === 401 && error.response.config.url !== "login/authenticate") {
    ...
    if (error.response.status === 422) {
      ...
      errors.forEach(error =>
        setTimeout(() => window._notification.error(error.msg, 450), 300)
      );
    }
    ...
    if (error.response.status === 400 || error.response.status === 403) {
      ...
      errors.map(res =>
        setTimeout(() => window._notification.error(res.message, 450), 300)
      );
    }
