...
const compareDate = (initDate, endDate) => {
  ...
  if (
    moment(inDate, "YYYY-MM-DD").isBefore(enDate, "YYYY-MM-DD") ||
    moment(inDate, "YYYY-MM-DD").isSame(enDate, "YYYY-MM-DD")
  ) {
    return true;
  }
  return false;
...
const diffHours = (date1, date2) => {
  date2 = !date2 ? moment().format("YYYY-MM-DD HH:mm:ss") : date2;
